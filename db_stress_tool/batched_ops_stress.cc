...
class BatchedOpsStressTest : public StressTest {
 public:
  ...
  Status TestPut(ThreadState* thread, WriteOptions& write_opts,
    ...
    std::string key_str = std::to_string(rand_keys[0]);
    for (int i = 0; i < 10; i++) {
      keys[i] += key_str;
      values[i] += v.ToString();
    ...
    if (FLAGS_opfile_path != "") {
      opfile_ << thread->tid << "," << op_count_[thread->tid] << ",BATCHED_PUT,"
              << rand_column_families[0];
      for (int i = 0; i < 10; i++) {
        opfile_ << "," << keys[i] << "," << value_base;
      }
      opfile_ << std::endl;
  ...
  Status TestDelete(ThreadState* thread, WriteOptions& writeoptions,
    ...
    if (FLAGS_opfile_path != "") {
      opfile_ << thread->tid << "," << op_count_[thread->tid] << ",BATCHED_DELETE,"
              << rand_column_families[0];
      for (int i = 0; i < 10; i++) {
        opfile_ << "," << keys[i];
      }
